...
// ===================== STEP 1 =====================
function ClientSignupStepOne({ nextStep }: { nextStep: (userId?: number) => void }) {
  ...
  useGetAllApi<ProvinceResponse>(ProvinceConfigs.resourceUrl, ProvinceConfigs.resourceKey, { all: 1 }, (res) => {
    setProvinceSelectList(res.content.map((item) => ({ value: String(item.id), label: item.name })));
  }, { refetchOnWindowFocus: false });

  useGetAllApi<DistrictResponse>(DistrictConfigs.resourceUrl, DistrictConfigs.resourceKey,
    { all: 1, filter: `province.id==${form.values['address.provinceId'] || 0}` },
    (res) => setDistrictSelectList(res.content.map((item) => ({ value: String(item.id), label: item.name }))),
    { refetchOnWindowFocus: false }
  );
  ...
  useGetAllApi<WardResponse>(WardConfigs.resourceUrl, WardConfigs.resourceKey,
    ...
    (res) => setWardSelectList(res.content.map((item) => ({ value: String(item.id), label: item.name }))),
    { refetchOnWindowFocus: false }
  );
...
function ClientSignupStepTwo({ nextStep, userId }: { nextStep: () => void, userId: number | null }) {
  const { updateCurrentSignupUserId } = useAuthStore();
  const [finalUserId, setFinalUserId] = useState<number | null>(userId);
  ...
  const confirmApi = useMutation<void, ErrorMessage, { userId: number, code: number }>(
